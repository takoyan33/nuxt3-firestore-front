...
function redirectToEditPage () {
  router.push(`/todos/edit/${route.params.id}`)
}
</script>
    ...
    <v-hover>
      <div variant="outlined" class="col-md-4 my-4 mb-8 px-4">
        ...
        <p
          class="my-2"
          :class="{
            'text-decoration-line-through': formattedDate > todo.period,
          }"
        >
        ...
        <span
          ...
          v-bind="props"
          :class="{
            'bg-deep-orange-darken-3 mx-3': todo.done === true,
            'bg-deep-purple-darken-3 mx-3': todo.done === false,
          }"
        >
          <span
            class="text-white font-weight-bold px-4 py-4 rounded-lg cursor-pointer mx-3"
            >{{ todo.done ? "完了" : "未完了" }}</span
          >
        </span>
        ...
        <div class="m-2">
          <v-btn variant="outlined" class="my-2" @click="redirectToEditPage"> 編集する </v-btn>
